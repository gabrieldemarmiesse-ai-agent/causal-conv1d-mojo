...
apply_silu × has_state_indices × is_circular × weight_vec_aligned ×
state_contig × use_external_stream) compiles the static
``update/variant.mojo`` once via ``mojo build -D KEY=VALUE …`` and
...
def call_update(args: tuple, pre_dispatch: Callable[[], None] | None = None) -> None:
    ...
    # entry point destructures `args[31]` for it.
...
def _config_from_args(args: tuple) -> tuple:
    ...
    wdtype_code = args[23]
    width = args[25]
    # The weight vector load is sized by the *parameter* dtype, so its
    # alignment proof uses wdtype's element size — not x's, which can now
    # differ (fp16 activations with fp32 weights).
    w_element_size = _ELEMENT_SIZE[wdtype_code]
    weight_vec_bytes = w_element_size * width
    weight_vec_aligned = (
        width in (2, 4)
        and args[13] == 1  # w_w_stride
        and args[1] % weight_vec_bytes == 0
        and (args[12] * w_element_size) % weight_vec_bytes == 0
    )
    return (
        dtype_code,
        wdtype_code,
        ...
        bool(args[26]),  # has_state_indices
        bool(args[28]),  # is_circular
        weight_vec_aligned,
        args[16] == 1,  # state_l_stride
        # See fwd/_jit.py for why this is comptime, not runtime branch.
        bool(args[30]),  # use_external_stream (1 for CUDA, 0 for Metal)
    )


def _mod_name(config: tuple) -> str:
    (dt, wdt, w, hb, silu, hi, circ, wv, sc, ues) = config
    return (
