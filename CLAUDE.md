...
- **(c) kernel-time bench** — cuda: vs Tri Dao upstream with min+spread and
  ...
  *absolute* per-kernel GPU time read back from a `xctrace` Metal System
  ...
  the % rather than inventing one. SKU variants matter — the table
  distinguishes e.g. `H100 PCIe` (2.0 TB/s) from SXM (3.35 TB/s); longest
  device-name substring wins. A shape whose working set fits in L2/SLC
  can legitimately measure *above* DRAM peak — that reports as
  `memory-bound (cache-resident)` (>110% of peak) rather than pretending
  DRAM is the roofline. On metal the roofline % is only trusted
